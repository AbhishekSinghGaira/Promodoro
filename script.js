...
// Load Spotify playlist from localStorage
function loadSpotifyPlaylist() {
    const spotifyKey = `pomodoro${timer.goal}${timer.userName}Spotify`;
    const savedSpotifyEmbed = localStorage.getItem(spotifyKey);
    
    if (savedSpotifyEmbed) {
        // Display the saved Spotify playlist
        spotifyPlayerContainer.innerHTML = savedSpotifyEmbed;
    }
}

// Save Spotify playlist to localStorage
function saveSpotifyPlaylist(embedCode) {
    const spotifyKey = `pomodoro${timer.goal}${timer.userName}Spotify`;
    localStorage.setItem(spotifyKey, embedCode);
}
    ...
    try {
        const response = await fetch('https://accounts.spotify.com/api/token', {
            ...
            headers: {
                ...
                'Authorization': 'Basic ' + btoa(SPOTIFY_CLIENT_ID + ':' + 'b16dc11aff61409087e972e023d03c69')
            },
...
// Update UI based on Spotify connection status
function updateSpotifyUI(isConnected) {
    if (isConnected) {
        ...
        setTimeout(() => {
            spotifyPlayerSection.classList.add('visible');
        }, 100);
    } else {
        ...
        spotifyPlayerSection.classList.remove('visible');
    }
}
...
// Update the getSpotifyEmbedCode function to use authentication
function getSpotifyEmbedCode(input) {
    // Check if input is already an embed code
    ...
    } else if (input.match(/^[a-zA-Z0-9]{22}$/)) {
        // Input is just the playlist ID
        playlistId = input;
    }
    
    if (playlistId) {
        return `<iframe src="https://open.spotify.com/embed/playlist/${playlistId}?utm_source=generator" 
...
function addSpotifyPlaylist() {
    const input = spotifyPlaylistInput.value.trim();
    
    if (!input) {
        alert('Please enter a Spotify playlist URL or embed code.');
        return;
    }
    
    const embedCode = getSpotifyEmbedCode(input);
    
    if (embedCode) {
        // Display the Spotify playlist
        spotifyPlayerContainer.innerHTML = embedCode;
        
        // Save to localStorage
        saveSpotifyPlaylist(embedCode);
        
        // Clear input field
        spotifyPlaylistInput.value = '';
    } else {
        alert('Invalid Spotify playlist URL or embed code. Please check and try again.');
    }
}
...
function setupEventListeners() {
    ...
    addSpotifyBtn.addEventListener('click', addSpotifyPlaylist);
    themeToggle.addEventListener('click', toggleTheme);
    setupSubjectPractice();
    
    // Also add event listener for Enter key on Spotify input
